...
Evaluates: visual weight balance, focal hierarchy, breathing room, rule-of-thirds.
"""
...
def _weight_balance(layout: ComposedLayout) -> float:
    ...
    Compares left/right and top/bottom weight sums.
    """
    left_weight = 0.0
    right_weight = 0.0
    top_weight = 0.0
...
def _breathing_room(layout: ComposedLayout) -> float:
    """Score how much whitespace exists between zones (no overlap penalty)."""
...
def _rule_of_thirds(layout: ComposedLayout) -> float:
    ...
    # Check the highest-weight placement
...
def score_layout(layout: ComposedLayout) -> float:
    """Composite heuristic score (0-1). Higher is better."""
    weights = {
        "balance": 0.25,
        "hierarchy": 0.30,
        "breathing": 0.25,
